...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, activeContext, user, onLogout }) => {
  const menuItems: Array<{ id: string; label: string; icon: any; beta?: boolean }> = [
    { id: 'generation', label: 'ARIA Generation', icon: Cpu },
    { id: 'chat', label: 'ARIA Chat', icon: MessageSquare, beta: true },
        ...
        <nav className="flex-1 mt-6 px-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => (
            ...
            >
              ...
              {item.beta && (
                <span className="px-1.5 py-0.5 bg-amber-500/20 text-amber-300 text-[9px] font-bold rounded uppercase tracking-wider">
                  Beta
                </span>
              )}
            </button>
