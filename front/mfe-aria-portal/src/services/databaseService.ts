...
export async function createIntakeRequest(intake: IntakeRequest): Promise<IntakeRequest> {
  console.log('💾 [DB] Creating intake request:', intake.id);
  try {
    const result = await apiFetch<IntakeRequest>(`${API_BASE}/intakes`, {
      method: 'POST',
      body: JSON.stringify(intake),
    });
    console.log('✅ [DB] Intake created in PostgreSQL:', result.id);
    return result;
  } catch (error: any) {
    console.error('❌ [DB] API error, falling back to localStorage:', error.message);
    const stored = getFromStorage<IntakeRequest[]>('aria_intake_requests', []);
    stored.push(intake);
    saveToStorage('aria_intake_requests', stored);
    return intake;
  }
}
...
export async function getIntakeRequestById(id: string): Promise<IntakeRequest | null> {
  console.log('📥 [DB] Fetching intake request:', id);
  try {
    const all = await getAllIntakeRequests();
    return all.find(r => r.id === id) || null;
  } catch (error: any) {
    console.error('❌ [DB] Error fetching intake request:', error.message);
    return null;
  }
}

export async function updateIntakeRequest(id: string, updates: Partial<IntakeRequest>): Promise<IntakeRequest> {
  console.log('📝 [DB] Updating intake request:', id);
  try {
    const result = await apiFetch<IntakeRequest>(`${API_BASE}/intakes/${id}`, {
      method: 'PUT',
      body: JSON.stringify(updates),
    });
    console.log('✅ [DB] Intake updated in PostgreSQL:', result.id);
    return result;
  } catch (error: any) {
    console.error('❌ [DB] API error, falling back to localStorage:', error.message);
    const stored = getFromStorage<IntakeRequest[]>('aria_intake_requests', []);
...
// ========================================
// LEGACY - Convert Intake to Initiative
// ========================================

export async function convertIntakeToInitiative(intakeId: string): Promise<Initiative> {
  console.log('🔄 [DB] Converting intake to initiative:', intakeId);
  
  const intake = await getIntakeRequestById(intakeId);
  if (!intake) throw new Error(`Intake request ${intakeId} not found`);

  const initiative: Initiative = {
    id: `INIT-${Date.now()}`,
    name: intake.title || intake.problem.substring(0, 100),
  ...
  const created = await createInitiative(initiative);
  await updateIntakeRequest(intakeId, { status: 'G1_Framing' });

  console.log('✅ [DB] Intake converted to initiative:', created.id);
  return created;
}
