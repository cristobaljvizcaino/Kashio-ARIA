...
export async function migrateLocalStorageToDb(): Promise<void> {
  ...
  try {
    ...
    // Migrate intake requests
    const localIntakes = getLocal('aria_intake_requests');
    if (localIntakes.length > 0) {
      const dbIntakes = await fetchJson('/api/db/intakes');
      const dbIds = new Set(dbIntakes.map((i: any) => i.id));
      
      let migratedCount = 0;
      for (const intake of localIntakes) {
        if (!dbIds.has(intake.id)) {
          try {
            await postJson('/api/db/intakes', intake);
            migratedCount++;
            console.log(`  ✅ Migrated intake: ${intake.id}`);
          } catch (e: any) {
            console.error(`  ❌ Failed to migrate intake ${intake.id}:`, e.message);
          }
        }
      }
      if (migratedCount > 0) {
        console.log(`🔄 [Migration] Migrated ${migratedCount} intake requests`);
      }
    }
