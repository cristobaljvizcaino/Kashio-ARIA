...
/** Prefijo de ruta del servicio (p. ej. ingress `/karia-svc/v2`). Vacío = raíz. */
const SERVICE_BASE_PATH = (process.env.SERVICE_BASE_PATH || '').replace(/\/$/, '');
const mountPath = SERVICE_BASE_PATH || '/';
...
routes.get('/api/library/files', async (req, res) => {
  try {
...
routes.get('/api/artifacts/files', async (req, res) => {
  try {
...
routes.post('/api/library/upload-url', async (req, res) => {
  try {
...
routes.get('/api/library/download/:fileId', async (req, res) => {
  try {
...
routes.delete('/api/library/delete/:fileId', async (req, res) => {
  try {
    const fileId = req.params.fileId;
    const [files] = await bucket.getFiles();
    const file = files.find(f => f.name.includes(fileId));
...
routes.post('/api/artifacts/publish', async (req, res) => {
  try {
...
routes.post('/api/artifacts/publish-pdf', express.raw({ type: 'application/pdf', limit: '50mb' }), async (req, res) => {
  try {
...
routes.get('/api/db/health', async (req, res) => {
...
routes.get('/api/db/initiatives', async (req, res) => {
  try {
...
routes.post('/api/db/initiatives', async (req, res) => {
  try {
    const init = req.body;
...
routes.put('/api/db/initiatives/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
routes.delete('/api/db/initiatives/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
routes.get('/api/db/intakes', async (req, res) => {
  try {
...
routes.get('/api/db/artifact-definitions', async (req, res) => {
  try {
...
routes.post('/api/db/artifact-definitions', async (req, res) => {
  try {
    const def = req.body;
    const result = await query(
...
routes.put('/api/db/artifact-definitions/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const updates = req.body;
...
routes.delete('/api/db/artifact-definitions/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
// Montaje bajo prefijo (ingress), p. ej. `/karia-svc/v2`. Raíz: `/`.
