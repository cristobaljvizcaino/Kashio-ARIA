...
| **Express** | Framework HTTP: rutas bajo `/karia-svc/v2/`, middleware JSON, raw body para PDF. |
...
Region habitual: **`us-central1`**. Las **8 Cloud Run Functions HTTP** del backend viven en **Kashio FinOps (`Kashio-Finops`)**: 4 IA (`functions/api`) y 4 biblioteca (`functions/library`). El **CRUD PostgreSQL** (iniciativas, definiciones de artefactos, lectura de intakes) lo expone el **servidor Express** (`src/index.ts`), normalmente desplegado como **Cloud Run** en el mismo ecosistema FinOps (ver §6 y §9).
...
+----------------------------------------------------------------------+
|  Cloud Run Functions - 8 endpoints HTTP (ver §7 y DEPLOYED_*.md)     |
...
+----------------------------------------------------------------------+
|  Secret Manager (ver §10)                                            |
|  * gemini-api-key (Gemini) -> Kashio-Finops                          |
|  * aria-database-url / URI BD -> proyecto del Cloud Run backend    |
...
## 4. Servidor Express (`src/index.ts`) — funciones lógicas por bloque
...
Convención: base URL = origen del backend (ej. `https://xxx.run.app` o `http://localhost:3000` en local; la imagen Docker/Cloud Run suele usar `8080`). Las rutas de negocio viven bajo el prefijo fijo **`/karia-svc/v2/`** (definido en `src/index.ts`). **`GET /health`** queda en la raíz para probes.
...
|--------|------|:-------------:|:---:|
| GET | `/health` | No | No |
| GET | `/karia-svc/v2/health` | No | No |
| GET | `/karia-svc/v2/health/db` | Solo **test** (`SELECT 1`) | No |

### 6.2 Biblioteca y artefactos (Google Cloud Storage + futuro `library_file`)

| Método | Ruta | Acción | Tablas SQL (hoy) |
|--------|------|--------|------------------|
| GET | `/karia-svc/v2/library/files` | Lista objetos del bucket (filtra versiones Output `.md`) | Ninguna |
| GET | `/karia-svc/v2/artifacts/files` | Lista nombres bajo prefijo `Output/` | Ninguna |
...
| Método | Ruta | Tabla |
|--------|------|--------|
| GET | `/karia-svc/v2/intakes` | `intake_request` (solo lectura) |
...
**Estado actual:** las **8 funciones HTTP** del backend viven en **Kashio-Finops** (`us-central1`), con URLs `https://{nombre}-476648227615.us-central1.run.app`. Código fuente: `functions/api/index.js` (4 IA) y `functions/library/index.js` (4 biblioteca). El **CRUD de datos** (iniciativas, definiciones, lectura de intakes) está en el **Express** `src/index.ts` (§6), no en estas funciones.
...
Operaciones directas sobre el bucket de biblioteca (por defecto **`karia-library-files`**). Alternativa a los mismos endpoints de Express bajo `/karia-svc/v2/`.
...
> **Nota (consola GCP):** si una función muestra advertencia (triángulo amarillo), puede haber una revisión pendiente o una variable de entorno no definida en esa revisión; la función puede seguir respondiendo.

### 7.3 Prueba manual de funciones (Kashio-Finops)

Las **8 funciones** tienen el patrón de URL `https://{nombre}-476648227615.us-central1.run.app`. Hoy están desplegadas con `--allow-unauthenticated` (para validación end-to-end), por lo que un curl simple basta:
...
Cuando IAM exija autenticación, el mismo patrón (método correcto + `Authorization: Bearer` + JSON) aplica a **todas** las funciones desplegadas en FinOps.

### 7.4 Despliegue en **Kashio FinOps** (Cloud Run Functions Gen 2, Node 22) — ✅ ejecutado
...
| Cambio | Detalle |
|--------|---------|
| `PROJECT_ID` | Resolución dinámica: `process.env.PROJECT_ID`, luego `GOOGLE_CLOUD_PROJECT`, por defecto `kashio-finops`. |
...
- **Producción:** retirar el flag y exigir IAM + Bearer token (ver §7.3).
...
### 8.1 Servidor Express (`node dist/index.js`)

| Variable | Obligatoria | Uso real en código |
|----------|-------------|---------------------|
| **`PORT`** | No | Puerto HTTP (default **3000** en local). Cloud Run / imagen Docker: `8080`. |
| **`ConnectionString_Karia`** | Para rutas bajo `/karia-svc/v2/` que tocan Postgres | URI PostgreSQL (`postgresql://…`), **conexión directa** al servidor (TCP/TLS). Sin ella el servidor **arranca**, pero rutas DB fallan (`Database not configured`). |
...
### 8.3 Prefijo de ingress (fijo en código)

El prefijo **`/karia-svc/v2/`** está **hardcodeado** en `src/index.ts` (`app.use('/karia-svc/v2/', routes)`). Para cambiarlo hay que editar ese archivo y redeployar.

- Las rutas de negocio quedan bajo **`/karia-svc/v2/...`** (ver §6).
- **`GET /health`** permanece en la **raíz** del contenedor para probes de Cloud Run.

**Frontend:** alinear los `fetch` del portal con el prefijo real (`/karia-svc/v2/initiatives`, etc.).

### 8.4 PostgreSQL directo y TLS (`src/config/database.ts`)
...
La imagen final **solo copia** `dist/` (salida de `tsc`). Las rutas que sirvan un SPA desde `dist/` **no están montadas** en este servicio salvo que lo agreguéis. En la práctica este backend se usa como **API** y el frontend va en otro hosting (Cloud Run aparte, CDN, etc.).
...
| **Variable de entorno en Cloud Functions** | `PROJECT_ID=kashio-finops` (se inyecta con `--set-env-vars` al desplegar; ver §7.4) |

**Código (`functions/api/index.js`):** la ruta usa `PROJECT_ID` con orden de resolución `PROJECT_ID` → `GOOGLE_CLOUD_PROJECT` → default **`kashio-finops`** (ID alfanumérico). En despliegues sobre FinOps `GOOGLE_CLOUD_PROJECT` suele venir poblado por GCP; aun así, el `--set-env-vars=PROJECT_ID=kashio-finops` en el despliegue (§7.4) lo deja explícito.

### 10.2 Otros secretos (CI/CD e integraciones)

Además de **`gemini-api-key`**, suelen existir secretos para **cadena PostgreSQL**, **Cloud Build** y **integraciones** (tokens de repositorio, webhooks, etc.). **Confirmá en Google Cloud Console** en qué proyecto está cada secreto (debe coincidir con el proyecto donde corre el consumidor: Cloud Run, Cloud Build, etc.).
...
- **Solo API sin BD:** sin `ConnectionString_Karia`; funcionarán rutas GCS si hay credenciales; las rutas Postgres bajo `/karia-svc/v2/` fallarán hasta configurar Postgres.
