...
| `GET /karia-svc/v2/artifact-definitions` | `artifact_definition` | `SELECT` filtrado/ordenado; respuesta agrupada por **fase** 1–8 | **Catálogo de artefactos** (KashioOS): `phase`, `public_id`, `predecessor_names` jsonb, etc. Ver **`API_DATABASE_ENDPOINTS.md`** §4. |
| `GET /karia-svc/v2/artifact-definitions/:publicId` | `artifact_definition` | `SELECT … WHERE public_id` | Detalle por UUID estable. |
| `POST /karia-svc/v2/artifact-definitions` | `artifact_definition` | `INSERT …` | Alta de definición (`phase` / `phaseName`, `predecessorNames`, …). |
